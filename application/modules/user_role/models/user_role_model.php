...
class User_role_model extends MY_Model {
	...
	function initialize()
	{
		$this->_configure 	= $this->config->item('user_role_configure');
		$this->_pagination 	= $this->config->item('pagination');
		$this->_validation 	= $this->config->item('user_role_validation');
	}
	...
	function ifExists($data, $elm_val, $edit_id = "")
	{
		if($edit_id)
		{
			...
			$query = $this->db->get_where($this->_configure['table_name'], array( $this->_configure['uc_id'] => $elm_val, $this->_configure['main_col'] => $data, $this->_configure['permanent_delete'] => 0 ));
		}
		else
			$query = $this->db->get_where($this->_configure['table_name'], array($this->_configure['uc_id'] => $elm_val, $this->_configure['main_col'] => $data, $this->_configure['permanent_delete'] => 0 ));
...
// END User Role Model Class

/* End of file user_role_model.php */
/* Location: ./application/modules/user_role/models/user_role_model.php */