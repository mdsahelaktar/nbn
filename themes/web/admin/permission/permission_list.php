...
if (!$response["event"]["error"]):
    ...
    <?php if ($edit_id): ?>
        ...
        if (isset($results["records"][0]->ai_permission_id)):
            ...
            echo form_hidden('row_id', $results["records"][0]->ai_permission_id);
            ?> <?php echo form_label('<p>' . _e('Permission Group') . '</p>', 'group_id') ?> <?php echo form_dropdown('group_id', $var['permissions_dd'], $results["records"][0]->group_id, 'id="group_id" class="validate" data-display="' . _e('Permission Group') . '" data-rules="required"');
    ...
    <?php else: ?>
        ...
        <table>
            ...
            <tr id="filter_section">
                ...
                <td><?php echo anchor('admin/permission/edit', _e('Reset'), array('title' => _e('Reset'), 'class' => 'refresh btn-type1 margin-right10 margin-left20 float-left')) ?> <?php echo form_button(array('name' => 'search', 'id' => 'search', 'class' => 'filter', 'onclick' => 'return goFilter(this)'), _e('Filter')); ?></td>
            </tr>
            <tr id="blanktr">
                <td colspan="6">&nbsp;</td>
            </tr>  
        ...
        if (count($results["records"])) {
            ...
            <?php foreach ($results["records"] as $value): ?>
                    <tr id="oddeven">
                        ...
                        <td align="center"><a href="<?php echo createEditUrl($value->ai_permission_id) ?>" class="ion-edit"></a>&nbsp;| &nbsp;<a href="<?php echo createRevertDeleteUrl($value->ai_permission_id, $value->is_trashed) ?>" class="editrevert<?php echo $value->is_trashed ?>"></a></td>
                    </tr>
            <?php endforeach; ?>
                <tr>
