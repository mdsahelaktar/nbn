...
if (!$response["event"]["error"]):
    ...
    <?php if ($edit_id): ?>
        ...
        if (isset($results["records"][0]->ai_theme_id)):
            ?>
      <div class="formcont-small">      <?php
            echo form_open('', array('name' => 'theme_edit_form', 'id' => 'theme_edit_form', 'class' => 'width100 float-left margin-bottom30 margin-top10'));
            echo form_hidden('row_id', $results["records"][0]->ai_theme_id);
            ?> <?php echo form_label('<p>' . _e('Theme') . '</p>', 'theme') ?> <?php echo form_input(array('name' => 'theme', 'id' => 'theme', 'size' => 30, 'value' => $results["records"][0]->theme, 'placeholder' => 'add theme here', 'class' => 'validate', 'data-display' => _e('Theme'), 'data-rules' => 'required')); ?>
           <div class="current_them"> <?php echo form_checkbox(array('name' => 'is_current', 'id' => 'is_current'), 1, $results["records"][0]->is_current ? True : False ); ?>
            <?php echo form_label('<p>' . _e('Make Current Theme') . '</p>', 'is_current') ?></div>
            
            <div class="clear"></div>
            <?php echo form_submit(array('name' => 'theme_update', 'id' => 'theme_update', 'class' => 'margin-top10'), _e('Update')); ?> <?php echo form_close() ?></div>
            <?php
    ...
    <?php else: ?>
        ...
        <?php echo form_open('', array('name' => 'theme_search_form', 'id' => 'theme_search_form', 'class' => 'width100 float-left margin-bottom30 margin-top10', 'method' => 'get')) ?>
        <table>
            <tr id="caption_section">
                <th><a href="<?php echo sortBy($var['CFG'], 'sortbytheme') ?>" class="<?php echo sortClass($var['CFG'], 'sortbytheme') ?>"><?php echo _e('Theme') ?></a></th>
                <th><?php echo _e('Current Theme') ?></th>
            ...
            <tr id="filter_section">
                <td><?php echo form_input(array('name' => 'theme', 'id' => 'theme', 'placeholder' => 'search by theme', 'size' => 20, 'value' => $this->input->get('theme'))); ?></td>
                <td><?php echo form_dropdown('is_current', $var['theme_status_dd'], $this->input->get('is_current') != '' ? $this->input->get('is_current') : '' );
        ?></td>
                <td></td>
                <td></td>
                <td><?php echo form_dropdown('is_trashed', $var['filter_status_dd'], $this->input->get('is_trashed') != '' ? $this->input->get('is_trashed') : '' );
            ?></td>    
                <td><?php echo anchor('admin/theme/edit', _e('Reset'), array('title' => _e('Reset'), 'class' => 'refresh btn-type1 margin-right10 margin-left20 float-left')) ?> <?php echo form_button(array('name' => 'search', 'id' => 'search', 'class' => 'filter', 'onclick' => 'return goFilter(this)'), _e('Filter')); ?></td>
            </tr>
            <tr id="blanktr">
                <td colspan="6">&nbsp;</td>
            </tr>
        ...
        if (count($results["records"])) {
            ...
            <?php foreach ($results["records"] as $value): ?>
                    <tr id="oddeven">
                        <td><label><?php echo form_checkbox('row_id[]', $value->ai_theme_id); ?><?php echo $value->theme ?></label></td>
                        ...
                        <td class="addedit" align="center"><a href="<?php echo createEditUrl($value->ai_theme_id) ?>" class="ion-edit"></a>     | &nbsp;<a href="<?php echo createRevertDeleteUrl($value->ai_theme_id, $value->is_trashed) ?>" class="editrevert<?php echo $value->is_trashed ?>"></a></td>
                    </tr>
            <?php endforeach; ?>
                <tr>
                ...
                <script type="text/javascript">
                    var deleteconformstring = '<?php echo _e('Confirm delete theme') ?>';
                    var pdeleteconformstring = '<?php echo _e('Confirm permanent delete theme') ?>';
                </script>
