...
<div id="body">
  <div class="wrapper">
    ...
    <fieldset class="clonefieldset">
      ...
      <div class="clear"></div>
      <?php echo form_label( '<p>'._e( 'Company Details' ).'</p>', 'company_details' )?> <?php echo form_textarea( array( 'name' => 'company_details', 'value' => $company_details, 'id' => 'company_details', 'placeholder' => 'add company details here', 'rows' => 5, 'autofocus' => 'autofocus') );?>
      <div class="clear"></div>
      <?php echo form_label( '<p>'._e( 'Broker Bio' ).'</p>', 'bio' )?> <?php echo form_textarea( array( 'name' => 'bio', 'value' => $bio, 'id' => 'bio', 'placeholder' => 'add broker bio here', 'rows' => 5, 'autofocus' => 'autofocus') );?>
      <div class="clear"></div>
      <div class="clear"></div>
    </fieldset>
    <fieldset class="clonefieldset">
      ...
      <?php echo form_label( '<p>'._e( 'Country' ).'</p>', 'country_id_broker_profile' )?> <?php echo form_dropdown('country_id', $var['country_dd'], $country_id, 'id="country_id_broker_profile" onchange="getProvinceByCountry(this)" data-province-sel="#province_id_broker_profile" data-child-input-parent="label[for=\'province_id_broker_profile\'], label[for=\'county_id_broker_profile\'], label[for=\'city_id_broker_profile\'], label[for=\'zipcode_broker_profile\']" data-child-input=":input#province_id_broker_profile, :input#county_id_broker_profile, :input#city_id_broker_profile, :input#zipcode_broker_profile" style="max-width:40%;" data-display="'._e( 'Country' ).'" data-rules="required"'); ?>
      <div class="clear"></div>
      <?php echo form_label( '<p>'._e( 'Province' ).'</p>', 'province_id_broker_profile') ?> <?php echo form_dropdown('province_id', $provinces_dd, $province_id, 'id="province_id_broker_profile" data-county-sel="#county_id_broker_profile" onchange="getCountyByProvince(this)" class="validate" data-display="'._e( 'Province' ).'" data-rules="required"');?>
      <div class="clear"></div>
      <?php echo form_label( '<p>'._e( 'County' ).'</p>', 'county_id_broker_profile' )?> <?php echo form_dropdown('county_id', $counties_dd, $county_id, 'id="county_id_broker_profile" data-city-sel="#city_id_broker_profile" onchange="getCityByCounty(this)" class="validate" data-display="'._e( 'County' ).'" data-rules="required"');?>
      <div class="clear"></div>
      <?php echo form_label( '<p>'._e( 'City' ).'</p>', 'city_id_broker_profile' )?> <?php echo form_dropdown('city_id', $cities_dd, $city, 'id="city_id_broker_profile" data-zip-sel="#zipcode_broker_profile" onchange="getZipByCity(this)" class="validate" data-display="'._e( 'City' ).'" data-rules="required"');?>
      <div class="clear"></div>
      <?php echo form_label( '<p>'._e( 'Zipcode' ).'</p>', 'zipcode_broker_profile' )?> <?php echo form_dropdown('zipcode', $zipcodes_dd, $zipcode, 'id="zipcode_broker_profile" class="validate" data-display="'._e( 'Zipcode' ).'" data-rules="required"');?>
      <div class="clear"></div>
    </fieldset>
    <fieldset class="clonefieldset">
      <span class="otherbutton"></span>
      <legend align="center" margin="auto"> <b>Certified Image</b></legend>
      <?php echo form_label( '<p>'._e( 'Images' ).'</p>', 'images' )?> <?php echo form_upload( array( 'name' => 'images[]', 'placeholder' => 'add images') );?>
      <div class="clear"></div>
    </fieldset>
