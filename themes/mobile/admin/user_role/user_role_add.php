...
<div class="formcont-small"><?php echo form_open( '', array( 'name' => 'user_role_add_form', 'id' => 'user_role_add_form', 'class' => 'width100 float-left margin-bottom30 margin-top10' ) )?> <?php echo form_label( '<p>'._e( 'User Category' ).'</p>', 'user_category_id' )?> <?php echo form_dropdown('user_category_id', $var['categories_dd'], $this->input->get('user_category_id'), 'id="user_category_id" class="validate" data-display="'._e( 'User Category' ).'" data-rules="required"');
...
<div class="clear"></div>
<?php echo form_submit( array( 'name' => 'user_role_add', 'id' => 'user_role_add', 'class' => 'margin-top10' ), _e( 'Add' ) );?> <?php echo form_close()?></div>
<?php
