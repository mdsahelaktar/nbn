...
if (!$response["event"]["error"]):
    ...
    <?php else: ?>
        ...
        ?></td>
                <td><?php echo anchor('admin/image/edit', _e('Reset'), array('title' => _e('Reset'), 'class' => 'refresh margin-left20 btn-type1 float-left')) ?> <?php echo form_button(array('name' => 'search', 'id' => 'search', 'class' => 'filter margin-left10', 'onclick' => 'return goFilter(this)'), _e('Filter')); ?></td>
            </tr>
            <tr id="blanktr">
                <td colspan="6">&nbsp;</td>
            </tr>  
            <tr id="multiple_action_section">
                <td colspan="9"><a onclick="return selectAll(this)" class="btn-type1 margin-right10 float-left" title="<?php echo _e('Select All') ?>" href="#"><span class="ion-checkmark-circled"></span><?php echo _e('Select All') ?></a> <a onclick="return unselectAll(this)" class="btn-type1 margin-right10 float-left" title="<?php echo _e('Unselect All') ?>" href="#"><span class="ion-close-circled"></span><?php echo _e('Unselect All') ?></a></td>
        ...
        if (count($results["records"])) {
            ...
            <?php foreach ($results["records"] as $value): ?>
                    <tr id="oddeven">
                        <td><label><?php echo form_checkbox('row_id[]', $value->ai_image_id); ?><?php echo $value->context_id ?></label></td>
                        <td align="center"><?php echo $value->relation_id ?></td>
                        <td><?php echo $value->caption ?></td>
                        <td align="center"><a href = "<?php echo $this->template->get_frontend_image($value->image_url) ?>" rel="lightbox"/> <img alt="" border="2%" src="<?php echo $this->template->get_frontend_image($value->image_url) ?>" width="40" height="30" rel="lightbox"/> </a>
                        <td align="center"><?php echo $value->is_main ? _e("Yes") : _e("No") ?></td>
                        <td align="center"><?php echo $value->alt ?></td>
                        ...
                        <td align="center"><a href="<?php echo createEditUrl($value->ai_image_id) ?>" class="ion-edit"></a>     | &nbsp;<a href="<?php echo createRevertDeleteUrl($value->ai_image_id, $value->is_trashed) ?>" class="editrevert<?php echo $value->is_trashed ?>"></a></td>
