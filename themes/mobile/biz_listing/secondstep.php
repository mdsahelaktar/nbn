...
<div id="stepsform">
  <div class="form_three-forth">
    <ul>
      ...
      <li> <?php echo form_label('<strong><dfn>' . _e('country') . '</dfn></strong>', 'country_id') ?> <?php echo form_dropdown('country_id', $var['countries_dd'], $results[0]->country_id, 'id="country_id" onchange="getProvinceByCountry(this)" data-province-sel="#province_id" class="validate" data-child-input-parent="#province_selector, #county_selector" data-display="' . _e('country') . '" data-rules="required"'); ?></li>
      <li id="province_selector"> <?php echo form_label('<dfn>' . _e('state_province') . '</dfn>', 'province_id', $location_dd_view_a) ?> <?php echo form_dropdown('province_id', $var['provinces_dd'], $results[0]->province_id, 'id="province_id" onchange="getCountyByProvince(this)" data-county-sel="#county_id" class="validate" data-display="' . _e('province') . '" data-rules="required" '.$location_dd_status.' '.$location_dd_view); ?></li>
      <li id="county_selector"> <?php echo form_label('<dfn>' . _e('county') . '</dfn>', 'county_id', $location_dd_view_a) ?> <?php echo form_dropdown('county_id', $var['counties_dd'], $results[0]->county_id, 'id="county_id" class="validate" data-display="' . _e('county') . '" data-rules="required" '.$location_dd_status.' '.$location_dd_view); ?> <br />
